        ...
        if (isset($_FILES['tpi_ip']['name'])) {
           ...
           // ...
            if (move_uploaded_file($_FILES['tpi_ip']['tmp_name'], $destination_path)) {
                ...
                echo "File uploaded successfully: $img_path";

                // Get the product_id from the form
                $product_id = $_POST['tpi_produk'];

                // Query to save data to the database
                $add = mysqli_query($database, "INSERT INTO product_images (`product_id`, `image_path`, `display_order`) VALUES ('$product_id', '$img_path', '$img_order')");
            ...
            echo "Error: File input 'tpi_ip' not found.";
        }
    }    
